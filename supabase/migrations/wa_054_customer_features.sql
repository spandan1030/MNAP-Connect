...
-- ── Walk-in HISTORY (wa_050). Before that migration only the latest visit was
--    kept, so counts start at 1 for anyone who visited earlier — the older
--    visits were overwritten and are genuinely gone, not merely unmigrated.
walkins AS (
  SELECT mnap_ten_digit(phone) AS phone,
    count(*)          AS visit_count,
    min(visited_at)   AS first_at,
    max(visited_at)   AS last_at,
    count(*) FILTER (WHERE NOT is_backfill) AS observed_count
  FROM wa_walkin_visits
  GROUP BY 1
),
...
SELECT
  ...
  -- ── app_ (customer app + product-interest tapped in chat) ──
  -- app_user / app_has_scheme come from the app-admin export (wa_053 feed);
  -- app_product_interest is set by the chat webhook on a "interested" / product
  -- link. Booleans are COALESCEd so a rule never trips over a NULL.
  COALESCE(ct.app_user, FALSE)                    AS app_is_user,
  COALESCE(ct.has_scheme, FALSE)                  AS app_has_scheme,
  COALESCE(ct.app_product_interest, FALSE)        AS app_product_interest,
  ct.app_product_interest_at                      AS app_product_interest_at,
  ...
  -- Visit history (wa_050). walkin_count was impossible before the visit log:
  -- each visit overwrote the last, so it could only ever have been 0 or 1.
  COALESCE(wk.visit_count, 0)                     AS walkin_count,
  wk.first_at                                     AS walkin_first_at,
  (COALESCE(wk.visit_count, 0) > 1)               AS walkin_is_repeat,
...
LEFT JOIN walkins     wk    ON wk.phone    = mnap_ten_digit(ct.phone)
