...
- **Test send.** `POST /api/invoices/test-send { phone, templateId }` fires the real template to one number with sample body values and a fake token (`…/i/test-preview`) — a WhatsApp preview with no real invoice, no publish, no ledger, opt-out not consulted.
