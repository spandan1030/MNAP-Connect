...
- **📊 Campaign report + per-bill tracking (`wa_063`, 2026-08-18).** Invoice sends are now recorded in the **Campaigns report** and get a per-bill engagement view.
  ...
  - **⏳ Phase 2 (deferred, chosen "core first"):** the two instrumented signals — **opened** (a client-side, post-hydration, write-once ping from `/i/[token]` → stamps `opened_at`; WhatsApp gives NO button-click callback, so page-open is the only "clicked" proxy) and **visited website** (click beacon on the Explore/scheme/contact links → `website_visited_at`). Both ride the existing `engage`→`feedback` secret bridge; build write-once to stay off the security backlog's flood surface.
...
- **🔭 Future plans.** (1) ✅ Production promotion DONE (live on `gold.mnalankarpalace.com/i`, Firestore TTL enabled 2026-08-18). (2) **Birthday/anniversary campaigns** — a rule on the new `contacts` month columns. (3) **Surface `wa_customer_feedback`** in an admin view. (4) **Pipeline automation** — emit `invoices_import.csv` (new bills only) so import isn't a manual raw-CSV upload. (5) **More Bill Summary sections** (featured products, schemes) via the registry. (6) **Invoice tracking Phase 2** — opened + visited-website instrumentation (see the tracking bullet above). (7) Security hardening backlog (`SECURITY_HARDENING_BACKLOG.md`).
