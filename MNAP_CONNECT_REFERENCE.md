...
**Invoice links — private per-bill "Bill Summary" pages (`wa_061` + `wa_062`, 2026-08-16, ✅ BUILT & staging-verified; production promotion pending):**
...
- **Feedback back-channel (Bill Summary page → Connect), `wa_062`.** The customer app's Bill Summary page captures a review (★) + birthday/anniversary month and posts them to `POST /api/invoices/feedback` (shared secret = `CUSTOMER_APP_PUBLISH_SECRET`, the SAME value as publish). It receives the invoice **token** (never a phone), maps token→phone via `wa_invoices`, then writes: `contacts.birthday_month` / `anniversary_month` (1–12, audience-targetable) and a `wa_customer_feedback` row (rating + reason for ≤3★). ≥4★ is handed to Google review on the customer side (no row). This is the reverse of publish: customer app → connect.
