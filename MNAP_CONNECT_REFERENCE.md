...
- **CTWA Conversions-API "engaged Lead" (`wa_068`, 2026-09-25):** turns Meta from
  ...
  the webhook fires **one** CAPI `LeadSubmitted` event to Meta, keyed on the stored `ctwa_clid`
...
- **`GET /api/campaigns`** unifies new runs + legacy broadcasts, newest first. **`/campaigns` page** (Navbar → More): list with sent/failed/suppressed, tap to expand the funnel bars. This folds the old broadcast report into one place.
  - **sent/failed are counted from `wa_send_ledger` (by `campaign_id`), NOT the stored `wa_campaigns.sent`/`failed` columns.** Those stored counters are a snapshot finalised *after* the send loop, so a large send that times out (serverless) after the messages went out + were ledgered — but before the counter update ran — left the row showing 0/partial despite real sends (seen live: "Daily-rate subscribers" 0 shown vs 280 sent). The list now self-heals by tallying the ledger (falls back to the stored counter only for pre-`campaign_id` runs with no ledger rows), matching the detail funnel which already counts from the ledger. The stored columns are still written but no longer trusted for display.
...
**Invoice links — private per-bill "Bill Summary" pages (`wa_061` + `wa_062`, 2026-08-16; ✅ BUILT & LIVE IN PROD, feedback back-channel prod-verified 2026-08-17):**
...
- **Built so far (mnap-connect ingest).** `wa_061_invoices.sql` → `wa_invoices` (one row per bill; `token` = 128-bit unguessable capability, generated once; `line_items` JSONB; lifecycle `sent_at`/`published_at`/`expires_at`; **insert-new-only by `bill_no`** so re-import never re-sends). `POST /api/invoices/import` **groups raw item rows by `RI_VRNO`** (header from first row, items accumulate), computes `payable = RI_NET_AMT − OA_AMT − AR_AMT`, parks bills with no valid phone. Column map: `RI_VRNO/RI_DATE/RI_CST_NAME/RI_PHN_NO/RI_AMT/RI_TAX_AMT/RI_NET_AMT/OA_AMT/AR_AMT` (header) + `ITM_NAME/PURT_NAME/RIO_NET_WT/BCM_BRCD/RIO_TOTAL_AMT/RIO_SALE_TYPE` (items; `RIO_TOTAL_AMT` is **tax-inclusive**, so items foot to `RI_NET_AMT`). Admin page `/admin/invoices/import` (SheetJS parse → **chunks on whole bills** so a request never splits a bill).
  - **Sale vs Return + null-item placeholders (2026-08-26).** `RIO_SALE_TYPE` classifies each item row: `Sale` (+) vs `Return` — a **new item the customer handed back** (a sales return, NOT old-gold exchange, which is the `OA_AMT` payment line). The bill total foots as `RI_NET_AMT = Σ Sale − Σ Return`, so a Return line is stored with a **negative** `amount`; the line items then sum to `RI_NET_AMT`. A row is an item line if it has **any** of name / barcode / amount / weight — a row with only bill-level totals (blank item detail) is header-only and contributes no line (these are the "no items" bills, awaiting a richer export query). An item row with a price but **no name** is kept with `item:null` (the Bill Summary page renders `item || "Item"`), so a multi-item bill never drops a priced line. Each line carries `sale_type:'sale'|'return'`. *Before this*: only `name||barcode` rows were kept, all as positives — so Return rows double-counted (item sum ≠ net, ~1.5% of bills, e.g. 1006 = 31900 − 16500 = 15400) and priced-but-unnamed rows vanished.
  - **Update / backfill mode.** `/admin/invoices/import` has an **"Update existing bills"** toggle → `POST /api/invoices/import { update:true }`. Default off = insert-new-only (unchanged). On = existing bills (by `bill_no`) get their **content** rewritten (items, amounts, name, date) while **`token` + send lifecycle (`sent_at`/`published_at`/`expires_at`/`created_at`) are preserved — nothing re-sends**; a **no-wipe guard** never replaces real items with an empty list (so re-uploading an older item-less export can't clobber good data). This is how the ~1.5% return/empty bills imported before the parser fix get corrected (re-upload with the box ticked). **Note:** an already-*sent* bill's live Firestore page isn't refreshed by a backfill (would need a re-publish-without-resend — not built).
  - **Send guard (2026-08-26).** `/api/invoices/send` **skips** a bill with **zero line items** (`skipped_no_items`) or a **net-negative** bill (`skipped_negative`, a pure refund) — left unsent, never published; surfaced on the Invoices tab as "N skipped (no items / refund)". Stops a customer ever receiving an item-less or refund Bill Summary.
  - **⚠️ SheetJS date gotcha (fixed 2026-08-18).** The import page MUST read with `{ cellDates:true, dateNF:'yyyy-mm-dd' }` (both on `XLSX.read` and `sheet_to_json`). Without it SheetJS auto-detects an ISO `RI_DATE` like `2018-09-14` and **reformats it to `9/14/18`** (2-digit year), which the server's `dateISO()` can't parse → **every `invoice_date` lands null** and the Invoices tab (which filters `invoice_date >= today−N`) shows **zero buyers**, silently. Root-caused when a 18,267-bill import came back 100% null-dated. `dateISO()` is now also liberal (Excel serials, `DD.MM.YYYY`, `DD-Mon-YYYY`), and the import result surfaces a **`datesNull` warning** so an all-null parse can't pass silently again. **Existing null-dated rows aren't self-healing** (insert-new-only by `bill_no`, and the raw date isn't stored) — they must be cleared (all unsent) and re-imported.
- **Send side — built into the existing thank-you module (mnap-connect).** A third **Invoices** tab on `/admin/thankyou` (Recent buyers + Send/test untouched). It reuses the **same Thank-you templates** as the other tabs (no separate category) — an invoice send just adds the dynamic button on top; the chosen Meta template must be a Utility template with a URL button `…/i/{{1}}`. Flow: pick template → **"Billed in the last N days"** filter (or tick **All dates** → `?days=all`, no recency window, for historical bills) + optional per-batch cap → Load → Send. Dedicated send path (NOT `dispatchTemplate`, which dedupes by phone + suppresses by template window — both wrong for invoices): `GET /api/invoices/pending?days=N` (unsent bills within N days, opt-out decorated) → `POST /api/invoices/send` sends **per invoice** (a customer's two bills = two links). Per bill, in order: skip if opted out → **`publishInvoice()` to the customer app FIRST** (fail ⇒ skip the send, so no dead link) → `sendTemplateMessage` with body params + `{type:button,sub_type:url,index:'0',parameters:[token]}` → thread + ledger (`category:'invoice'`, `suppression_key:'invoice:<bill_no>'`) → stamp `sent_at/published_at/expires_at = now+7d`. Suppression is `sent_at IS NULL`, not the resend window. Two server-only envs: `CUSTOMER_APP_PUBLISH_URL`, `CUSTOMER_APP_PUBLISH_SECRET`.
  - **⚠️ `CUSTOMER_APP_PUBLISH_URL` gotcha:** it drives BOTH where the snapshot publishes AND the report's copy-link base (`report/route.ts` strips `/api/…` off it, falls back to `gold.mnalankarpalace.com`). A **staging value silently publishes bills + issues links to the wrong place** (caught in prod 2026-08-18: report links were staging). Safeguard added: `GET /api/invoices/config` (admin) returns `{base,isProd,configured,hasSecret}`; the Invoices tab shows a red banner and **disables Send** when the URL is unset/non-prod or the secret is missing. Prod value = `https://gold.mnalankarpalace.com/api/invoices/publish`.
- **Test send.** `POST /api/invoices/test-send { phone, templateId }` fires the real template to one number with sample body values and a fake token (`…/i/test-preview`) — a WhatsApp preview with no real invoice, no publish, no ledger, opt-out not consulted. Body params are **pure positional** (`{{1}}`=name, `{{2}}`=bill_no) — no `parameter_name` (matches a positional Meta template). Template authoring rule: numbered `{{1}}`/`{{2}}` in Meta, value-keys in matching order in the connect "Variable names" field.
- **Feedback back-channel (Bill Summary page → Connect), `wa_062`.** ✅ **LIVE + prod-verified end-to-end (2026-08-17).** The customer app's Bill Summary page captures a review (★) + birthday/anniversary month and posts them to `POST /api/invoices/feedback` (shared secret = `CUSTOMER_APP_PUBLISH_SECRET`, the SAME value as publish). It receives the invoice **token** (never a phone), maps token→phone via `wa_invoices`, then writes birthday/anniversary to the **contacts spine** (the promotable customer book): UPDATE the existing contact's `birthday_month`/`anniversary_month` (1–12) — never touching name/opt-out/provenance; if **no contact exists yet**, CREATE a named, targetable one from the invoice (`billing_name` + `from_sales=true`) so a birthday never lands on a nameless row. Also inserts a `wa_customer_feedback` row (rating + reason for ≤3★). ≥4★ is handed to Google review on the customer side (no row). Reverse of publish: customer app → connect.
  - **⚠️ Critical fix (2026-08-17):** the auth `middleware.ts` redirected every unauthenticated request to `/login` except `/api/whatsapp/*`, so this **server-to-server (no cookie)** endpoint was being bounced to `/login` and **every submission silently discarded** — the back-channel had never actually stored anything. Now `middleware.ts` also exempts `/api/invoices/feedback` (self-authenticating via the shared secret); the admin `/api/invoices/{import,pending,send,test-send}` routes stay login-gated. Verified in prod via the customer `engage` endpoint → `feedback` → `contacts` (both update + create paths, name preserved).
  - **Test-preview no-op (2026-08-17):** the test-send publishes a sample bill under the fixed `test-preview` token but writes **no `wa_invoices` row** by design, so feedback from it used to 404 (`Unknown token`). `feedback` now short-circuits `token === 'test-preview'` to `{ ok: true, test: true }` — the preview's birthday/review UX completes without error and **nothing is written to a real contact** (the test lands on the admin's own number).
  - **🔒 Security hardening backlog:** the endpoints added here (public `engage`, secret-gated `feedback`/`publish`, `/i/[token]`) were audited 2026-08-17 — findings (no rate limiting, single static plaintext secret, DB-error disclosure, feedback tampering/flood, Referrer-Policy, TTL, timing-safe compare) and prioritised fixes are in **`SECURITY_HARDENING_BACKLOG.md`** (repo root). **Fixes deferred — work on later.**
- **Customer-app half (built, in `mnap-customer`, on `staging`).** `POST /api/invoices/publish` (shared secret) writes Firestore `invoices/{token}` (+ `expiresAtTs` for TTL); server route `/i/[token]` renders the branded **Bill Summary** (banner, bill, disclaimer footnote, print-to-PDF) + extensible **section registry** (GiftCard, Discover…). Privacy: `noindex` metadata + `X-Robots-Tag`/`no-store` header + Firestore client-deny + 7-day expiry. See `mnap-customer/PROJECT_REFERENCE.md`.
- **📊 Campaign report + per-bill tracking (`wa_063`, 2026-08-18).** Invoice sends are now recorded in the **Campaigns report** and get a per-bill engagement view.
  - **Rolling campaign:** the send route get-or-creates ONE `wa_campaigns` row (`category='invoice'`, name **"Invoice links"**, `is_dynamic=false`) and every send accumulates into it — stamps `campaign_id` on the ledger rows, upserts `wa_campaign_members` (deduped by phone), and bumps the summary counts. So it shows in `/api/campaigns` + the funnel (`/api/campaigns/detail`, source=reach: sent→delivered→read→replied→converted) with the SAME insights as Reach. (Design choice = one rolling campaign, not per-batch. Note: the detail's replied/converted windows anchor to the campaign's `created_at`, i.e. cumulative since the invoice program began; per-recipient `sentAt` is exact from the ledger.)
  - **Per-bill columns on `wa_invoices` (`wa_063`):** `wa_message_id` (outbound wamid → delivery/read via `wa_message_events`), `campaign_id`, `reviewed_at`/`review_rating`, `birthday_submitted_at`/`anniversary_submitted_at` (stamped by the feedback route), and schema-ready-but-unwired `opened_at`/`website_visited_at`.
  - **Report:** `GET /api/invoices/report?limit=300` → per-bill `{ url, delivered, read, opened, reviewed(+rating), birthday, anniversary, visitedWebsite }` + summary. Surfaced in the thank-you **Invoices tab → "Sent report"** (copy-link per bill, ✓/★ columns). Grained per BILL (a 2-bill customer = 2 rows/URLs) — that's why it's separate from the per-phone campaign funnel.
  - **✅ Phase 2 (built 2026-08-18):** the two instrumented signals. **opened** — customer `InvoiceTracker.tsx` (client) fires `{opened:true}` once on mount (post-hydration, so JS-less preview crawlers can't inflate it; WhatsApp gives NO button-click callback, so page-open is the "clicked" proxy) → feedback stamps `opened_at` **write-once** (`.is('opened_at', null)`). **visited website** — outbound Explore/scheme/contact anchors carry `data-visit`; a capture-phase delegated click (keepalive) fires `{visited:true}` → `website_visited_at` write-once. Both ride the same `engage`→`feedback` secret bridge (extended to pass `opened`/`visited`); localStorage-guarded client-side too. Write-once means re-open/re-click can't churn the row (addresses the backlog's feedback-flood note for these two). Surfaced as Opened/Site columns in the Sent report.
- **Data model + relations (the whole feature).**
  - `wa_invoices` (Connect/Supabase): one row per bill. **`token`** is the join key to Firestore `invoices/{token}` (Customer) AND the capability in the WhatsApp button URL. `phone` → `contacts.phone`. Engagement columns per `wa_063` (above).
  - Firestore `invoices/{token}` (Customer): the rendered snapshot (NO phone — privacy). Auto-purged by TTL on `expiresAtTs`.
  - `contacts.birthday_month` / `anniversary_month` + `wa_customer_feedback`: written by the feedback back-channel, keyed to phone via `wa_invoices` token lookup.
  - Two directions, one shared secret value (Customer `INVOICE_PUBLISH_SECRET` ≡ Connect `CUSTOMER_APP_PUBLISH_SECRET`): **publish** Connect→Customer (send time), **feedback** Customer→Connect (engagement).
- **Verified end-to-end on staging (2026-08-16):** publish→Firestore→render, positive/negative review paths, gift capture, `noindex` headers. Real WhatsApp test-send delivered with the dynamic button.
- **✅ Achievements.** Sources bill-level truth the aggregated Call Control feed had thrown away; personalised transactional (Utility) engagement without leaving the thank-you module; first **Customer→Connect** data back-channel (birthday/anniversary now audience-targetable); privacy-first capability-URL pattern (unguessable token, noindex, TTL, no phone in the browser).
- **🔭 Future plans.** (1) ✅ Production promotion DONE (live on `gold.mnalankarpalace.com/i`, Firestore TTL enabled 2026-08-18). (2) **Birthday/anniversary campaigns** — a rule on the new `contacts` month columns. (3) **Surface `wa_customer_feedback`** in an admin view. (4) **Pipeline automation** — emit `invoices_import.csv` (new bills only) so import isn't a manual raw-CSV upload. (5) **More Bill Summary sections** (featured products, schemes) via the registry. (6) ✅ Invoice tracking Phase 2 DONE (opened + visited-website, 2026-08-18). (7) Security hardening backlog (`SECURITY_HARDENING_BACKLOG.md`).
...
**QC fixes — audience/engagement logic (2026-09-15, no migration):** four live bugs, root-caused:
- **Stale dynamic membership.** Dynamic audiences froze their `audience_members` at creation and only re-synced on manual refresh (no daily job existed), so narrow/activate/report ran against a stale snapshot — narrowing "hot starred ★ ∩ called-in-30d" collapsed to a handful because most stars/calls happened *after* creation. Fix: `refreshAudienceMembers(id)` is now called on Insights open (`report`), on `activate`, and on `save-slice` narrow. New **`POST /api/audiences/refresh-all`** (signed-in user) re-materialises every active dynamic audience in one sweep, behind a **"Refresh all" button** on `/audiences` — press it to bring every live audience's member count current (fixed audiences are a no-op). Chosen over a cron: manual, visible, nothing to configure. The report is now fully derived — `total` from `wa_campaign_members`, `sent`/`failed` from the ledger, `delivered`/`read` from events, `replied` from inbound — trusting no stored `wa_campaigns` counter.
- **Profile hid individually-sent messages.** `CustomerPeek`'s "Messages sent" read only `wa_send_ledger`; one-off inbox sends (`/api/whatsapp/send`) write only `wa_messages`. Fix: peek UNIONs outbound `wa_messages` (complete record) with the ledger (category/campaign), matched on `wa_message_id`; ledger-only rows (e.g. failed campaign sends) still shown.
- **Activated chat campaign showed "0 sent".** `audiences/activate` called `dispatchTemplate` but never wrote `wa_campaigns.{sent,failed,total}` back (every sibling send path does); the report read the stored 0. Fix: activate now accumulates the counters, and the report derives sent/failed from the ledger so pre-existing campaigns self-heal.
- **Rule-builder count was whole-DB, not within-audience.** `/api/audiences/count` now takes an optional `audienceId`; when present the headline total is `members ∩ tree` and `RuleBuilder` shows "N in this audience match" (via a `scoped` flag). This is the number that now matches what a narrow actually saves. Per-rule/per-group hints stay global ("this rule alone"). **`audienceId` is passed by BOTH narrow builders** — Insights' NarrowSlice AND the Activate sheet's "Narrow further" (the latter fixed 2026-09-20; it was still showing the global count, e.g. 565 for a subset of a 200-member audience). Opening the Activate sheet also refreshes a dynamic audience so header/preview/send agree.

**Members preview + view + CSV export (2026-09-20, no migration):** one engine, `POST /api/audiences/members`, returns name+phone+opt-out for a cohort — `{audienceId, subRules?|subFilter?}` (an audience or its exact send set) or `{rules}` (any subset of the pool). Refreshes a dynamic audience first so preview == send set; returns `{count, sendable, optedOut, members[], truncated}` (sendable = count − opted-out; `limit:0` = full list for export, hard cap 50k). Reusable `components/audiences/MembersView.tsx` (search, tap→CustomerPeek, client-side CSV `Name,Phone,Opted out`). Wired into: **Activate sheet → "Preview recipients"** (chat, reflects the current narrow); **audience card → "View / export"** (whole audience); **create/edit builder → "Preview / export matches"** (arbitrary pool subset, Rules or convertible Chips). CSV download is a client blob — fine in the real app (only Artifacts sandbox blocks downloads).
