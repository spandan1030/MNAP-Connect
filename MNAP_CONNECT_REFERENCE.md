...
- **Thank-you "recent buyers" auto-mode** — new default tab on `/admin/thankyou`: `GET /api/thankyou/recent-buyers?days=14&templateId=` pulls buyers with `wa_b_markers.last_purchase_date` in the window (no upload), shows per-phone thank-you suppression, one-click sends via `/api/reach/send` (14-day guard). Legacy upload tabs kept.
  - **Templates umbrella (2026-07-15):** the thank-you picker no longer needs a separate `category='thankyou'` tag. It lists templates from the **Templates module** filed under the **"Thank You" / "Purchased" topic** (`wa_interest_topics`, matched by name `/thank|purchas/i`), with `category='thankyou'` kept only as a legacy fallback so already-tagged rows still show. In `/admin/templates`, choosing the Thank-you topic auto-sets the reach category to `thankyou` (visible in the amber Reach block, overridable). One place to define a thank-you message: Templates → assign the Thank You topic.
