...
- **Roster:** `salesmen` (name + short `alias`), managed in **Call Control**. Distinct from app-login accounts (`wa_b_call_logs.called_by` stays the device user). `wa_b_call_logs.salesman_id` + `wa_b_customers.walkin_salesman_id`/`walkin_at` added.
