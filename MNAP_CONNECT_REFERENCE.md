...
- **UI:** `/audiences` (Navbar → More) — list (member count, FIXED/AUTO-UPDATE badge, last-refreshed) + create/edit sheet + refresh + delete. **Filter builder extracted to `components/reach/FilterBuilder.tsx`** (the Call/Chat/Subscribed/Walk-in/Sales/Signal bands) and now **shared by Reach and Audiences** — one filter UI, no drift. Activation (send/call off a saved audience) + sub-filters are the next increment.
