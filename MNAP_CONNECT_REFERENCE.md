...
- **Stale dynamic membership.** Dynamic audiences froze their `audience_members` at creation and only re-synced on manual refresh (no daily job existed), so narrow/activate/report ran against a stale snapshot — narrowing "hot starred ★ ∩ called-in-30d" collapsed to a handful because most stars/calls happened *after* creation. Fix: `refreshAudienceMembers(id)` is now called on Insights open (`report`), on `activate`, and on `save-slice` narrow. New **`GET`/`POST /api/audiences/refresh-all`** (guarded by `CRON_SECRET`, else a signed-in user) re-materialises every active dynamic audience; **`vercel.json` runs it daily at 02:00 UTC** (Vercel Cron uses GET and auto-adds the `CRON_SECRET` bearer; route is `maxDuration=60`). **Owner TODO: set `CRON_SECRET` in Vercel env** or the scheduled run 401s. The report is now fully derived — `total` from `wa_campaign_members`, `sent`/`failed` from the ledger, `delivered`/`read` from events, `replied` from inbound — trusting no stored `wa_campaigns` counter.
