...
- **Vocabulary:** enforcement was already unified (`resolveCohortPhones`/`dispatch` apply the single `contacts.is_opted_out` once) — the audit confirmed the *terms* that looked like drift are principled distinctions (call-channel `is_do_not_call` vs unified `is_opted_out`; prospect **segment** vs **audience** vs run-**cohort**). Only fix: Reach's stale "Opted out (STOP)" label → "Opted out" (STOP is now one of three opt-out sources). `GLOSSARY.md` gained **Interval** and **Two faces (Rules / Chips)** as canon.
