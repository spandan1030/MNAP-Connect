...
- Still open (low risk, additive): the Call Control **UI** still shows its 12 bespoke chips. The API now accepts `rules`, so swapping that page onto the shared FilterBuilder/RuleBuilder is a UI-only follow-up.
