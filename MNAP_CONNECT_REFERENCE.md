...
- **Data model + relations (the whole feature).**
  - `wa_invoices` (Connect/Supabase): one row per bill. **`token`** is the join key to Firestore `invoices/{token}` (Customer) AND the capability in the WhatsApp button URL. `phone` → `contacts.phone`.
...
- **🔭 Future plans.** (1) **Production promotion:** merge `mnap-customer` `staging`→prod so `gold.mnalankarpalace.com/i` is live, set the prod-backend envs, enable the Firestore TTL policy, point the Meta template button at the prod domain. (2) **Birthday/anniversary campaigns** — a rule on the new `contacts` month columns. (3) **Surface `wa_customer_feedback`** in an admin view. (4) **Pipeline automation** — emit `invoices_import.csv` (new bills only) so import isn't a manual raw-CSV upload. (5) **More Bill Summary sections** (featured products, schemes) via the registry.
