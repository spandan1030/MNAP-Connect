...
- **Still to build.** mnap-customer: `POST /api/invoices/publish` (shared-secret) → Firestore `invoices/{token}` (+ `expiresAt` TTL); server route `/i/[token]` (Admin SDK read, `noindex`, expiry 410, PDF download). mnap-connect: **`dispatch.ts` URL-button component** (per-recipient token) + Utility template + `/admin/invoices` send screen (publishes at send time, `expires_at = now+7d`). Pipeline: emit `invoices_import.csv` (new bills only). Docs: DPDP + Play data-safety check.
