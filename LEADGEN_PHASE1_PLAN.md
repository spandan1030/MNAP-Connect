...
| D-subfilter | **Sub-filters at send time.** After saving an audience (e.g. "walked in 12–14 Jul"), a send can further narrow it — e.g. occasion (wedding/gift) or planning-to-buy (7d / 1–3m) — then send to chat OR push to calling, **without** creating a new saved audience. |
| D-callqueue | **One calling cohort at a time.** Pushing an audience/subset to calling **replaces** the current active calling set (the calling deck shows exactly one cohort). |
| D-profile | **Everything reflects on the profile (CustomerPeek):** which audiences/campaigns the person is in, what comms they got **in** each campaign, and what they got **outside** campaigns. The **ledger is written for every send** regardless, so attribution is never lost. |
| D-reports | **Reports show both** message insights (sent/delivered/read/replied/converted) **and** call insights (called / outcome) per audience — folding today's Call-Control report into the audience report. |
