  ...
  try {
    ...
    if (template?.meta_template_name) {
      ...
      const parameters = variables.map(varName => {
        ...
        // 'name' → named variable {{customer_name}} in Meta — needs parameter_name: 'customer_name'
        // rate variables → positional {{1}}, {{2}}… in Meta — must NOT have parameter_name
        if (v === 'name') {
          return { type: 'text', parameter_name: 'customer_name', text: customerName }
        }
